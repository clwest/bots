...
username = "0o8prpw37fzkhl3b6rxe4yznk"
...
def reduce_track_information(track_list):
    ...
    # get the list of tracks, handle search and show results
    if tracks is not None:
        # format of search track_list
        tracks = tracks["items"]
    else:
        #format of get saved track_list
...
def get_tracks_for_search_text(search_input):
    """
    Get tracks that match the given text and map them to 
    triplets containing the artist, song and track id.
    Parameters: 
    search_text - Name of the song and/or artist to be searched for the
...
def add_saved_track_by_id(track_id):
    """
    Add the track with the given id to the saved tracks listed
    
    Parameters: 
    track_id - id of the track to be added
    """
    write_scoped_sp.current_user_saved_tracks_add(tracks=[track_id])

def add_selected_track(found_tracks, selection):
    """
    Parse the given selection from the user and add the given track_id to the users saved track list.
    """
    ...
    try:
        ...
        print(f"Invlaid selection, please select a valid number... {err}")
...
def main():
    ...
    while running:
        ...
        print_options()
        user_choice = input("What do you want to do (choose a number)?\n")
